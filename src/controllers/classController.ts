...
export async function createClass(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const { studentId, date, numberOfStudents, duration, subject, type, modality } = req.body
...
export async function getClassesCashFlowSummary(req: Request, res: Response, next: NextFunction) {
  try {
    const role = (req as any).auth.role as UserRole
    const { startDate, endDate, tutorId, guardianId, studentId } = req.query
...
export async function getClassesDetails(req: Request, res: Response, next: NextFunction) {
  try {
    const role = (req as any).auth.role as UserRole
    const { startDate, endDate, tutorId, studentId, guardianId, page, pageSize } = req.query
...
export async function deleteClass(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (
      payment.guardianPaymentStatus !== PaymentStatus.pending ||
      payment.tutorPaymentStatus !== PaymentStatus.pending
    ) {
      return res.status(409).json({
        message: 'Class cannot be deleted because guardian or tutor payment is already completed',
      })
    }
...
export async function updateClassPaymentStatus(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const { guardianPaymentStatus, tutorPaymentStatus, guardianPaymentType } = req.body
...
export async function updateGuardianClassPaymentStatusByGuardianId(
  ...
  try {
    ...
    if (
      guardianPaymentStatus === PaymentType.bankTransfer ||
      guardianPaymentStatus === PaymentType.card
    ) {
