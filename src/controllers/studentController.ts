...
export async function addStudentToGuardian(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!institutionId || typeof institutionId !== 'number') {
      return res
        .status(400)
        .json({ ok: false, message: 'Institution ID is required and must be a number' })
    }

    if (!guardianId || typeof guardianId !== 'number') {
      return res
        .status(400)
        .json({ ok: false, message: 'Guardian ID is required and must be a number' })
    }
...
export async function removeStudentFromGuardian(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!studentId || typeof studentId !== 'number') {
      return res
        .status(400)
        .json({ ok: false, message: 'Student ID is required and must be a number' })
    }
    ...
    if (updateResult.count === 0) {
      return res
        .status(404)
        .json({ ok: false, message: 'Student not found for the specified guardian' })
    }
